...
- **Backfill:** `npm run db:seed-packaging:{dev,prod}` → `scripts/seed-item-packaging.ts` + `prisma/seed-data-templates/Item_Packaging.csv` (seed contract in [build-and-deploy.md](build-and-deploy.md#seeding)), **fill-only** unless `--overwrite`. The CSV's Source column records where each number came from (the morning-load sheet's `(24X1)` and the client's own pack codes). **Deliberately left empty:** SNICKERS' box (the sheet says 24, the pack code says 20, and that is the client's question to answer), the four `45X1` cakes (45 per box or 45 g — ambiguous), Pringles' size (`30/40G`), and items with no source (`ULKER SANDWICH`, `DEMAH BROWNIE`, `LIPTON PEACH ZRO`, `landessa`).
