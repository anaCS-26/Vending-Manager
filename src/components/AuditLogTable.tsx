...
export default function AuditLogTable({ initialResult, actionTypes, adminNames, driverNames }: Props) {
    ...
    const inputCls = "bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-brand-500/50 transition-colors";

    return (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
            {/* Filters */}
            <div className="p-4 border-b border-slate-800 flex flex-col lg:flex-row gap-3 lg:items-center">
            ...
            {/* Header row */}
            <div className="hidden md:flex px-4 py-3 border-b border-slate-800 text-[10px] font-bold uppercase tracking-widest text-slate-500 bg-slate-950/40">
            ...
            <div className="relative">
                ...
                {result.data.length === 0 ? (
                    ...
                    result.data.map(row => {
                        ...
                        return (
                            <div key={row.id} className="border-b border-slate-800/70 last:border-b-0">
                                <button
                                    onClick={() => hasState && setExpandedId(isOpen ? null : row.id)}
                                    className={`w-full text-left flex flex-col md:flex-row md:items-center px-4 py-3 gap-1 md:gap-0 transition-colors ${hasState ? 'hover:bg-white/5 cursor-pointer' : 'cursor-default'}`}
                                >
                                    <div className="w-40 shrink-0">
                                        <div className="text-sm font-medium text-slate-200">{formatSaudiDate(row.timestamp)}</div>
                                        <div className="text-[11px] text-slate-500 font-mono">{formatSaudiTime(row.timestamp)}</div>
                                    </div>
                                    <div className="w-40 shrink-0">
                                        <div className="text-sm font-medium text-white truncate">{resolveActor(row)}</div>
                                        <div className="text-[10px] uppercase tracking-wider text-slate-500">{row.actorRole}</div>
                                    </div>
                                    <div className="w-48 shrink-0">
                                        <span className="inline-block text-[11px] font-bold font-mono px-2 py-0.5 rounded-md bg-brand-500/10 text-brand-300 border border-brand-500/20">
                                            {row.actionType}
                                        </span>
                                        {row.entityType && (
                                    ...
                                    <div className="flex-1 min-w-0 text-sm text-slate-300 md:truncate">
                                        {row.message || <span className="text-slate-600 italic">—</span>}
                                    </div>
            ...
            {/* Footer */}
            <div className="flex flex-col sm:flex-row items-center justify-between px-4 py-4 border-t border-slate-800 gap-4">
