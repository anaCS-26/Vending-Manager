...
/**
 * Enables/disables Leaflet's own touch handlers after mount. MapContainer only
 * reads its interaction props once, at creation, so the "tap to interact" gate
 * below has to reach the map instance directly.
 */
function TouchGate({ enabled }: { enabled: boolean }) {
    const map = useMap();
    useEffect(() => {
        const handlers = [map.dragging, map.touchZoom, map.doubleClickZoom] as const;
...
export default function MapVisual({ machines, predictions = [], warehouses = [] }: { machines: any[], predictions?: any[], warehouses?: any[] }) {
    ...
    // A full-bleed Leaflet map is a scroll trap on a phone: one-finger drag pans
    // the map, so a thumb that lands anywhere on it cannot scroll the dashboard
    // past it. Touch devices start locked and opt in with a tap; pointer devices
    // (which scroll with a wheel Leaflet doesn't swallow) are interactive from
    // the start, so nothing changes on desktop.
    const [touchLocked, setTouchLocked] = useState(false);

    useEffect(() => {
        setIsMounted(true);
        setTouchLocked(window.matchMedia("(hover: none) and (pointer: coarse)").matches);
    }, []);
    ...
    return (
        <div className="w-full h-[280px] sm:h-[450px] rounded-3xl overflow-hidden relative z-0 border border-slate-200 dark:border-white/10 group shadow-2xl">
            ...
            {/* Touch scroll-trap gate — see `touchLocked` above. */}
            {touchLocked && (
                <button
                    type="button"
                    onClick={() => setTouchLocked(false)}
                    className="absolute inset-0 z-[600] flex items-end justify-center pb-4 bg-transparent"
                    aria-label="Enable map interaction"
                >
                    <span className="rounded-full bg-slate-900/80 dark:bg-black/80 text-white text-[11px] font-bold tracking-wide px-4 py-2 backdrop-blur-md border border-white/10 shadow-lg">
            ...
            {/* Map Legend */}
            <div className="absolute bottom-3 left-3 sm:bottom-6 sm:left-6 z-[500] bg-slate-100/90 dark:bg-slate-900/90 backdrop-blur-md border border-slate-200 dark:border-white/10 p-2 sm:p-3 rounded-xl shadow-2xl flex flex-col gap-1.5 sm:gap-2 pointer-events-none">
